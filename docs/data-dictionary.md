...
| owner_id | uuid (FK → users.id) | Sí | null | Dueño (null = anónimo) |
...
| is_claimed | boolean | No | false | Si fue reclamado por un usuario |
| status | enum(active, disabled, flagged) | No | active | Estado de moderación |
| expires_at | timestamp | Sí | null | Expiración (solo links anónimos, 24h) |
| created_at | timestamp | No | now() | Fecha de creación |
| updated_at | timestamp | No | now() | Última actualización (auto) |
...
| ip_hash | varchar(64) | Sí | null | SHA256 truncado (16 chars) de la IP |
| country | varchar(2) | Sí | null | Código de país ISO (ej. CO, US) |
| city | varchar(100) | Sí | null | Ciudad (de Vercel GeoIP) |
...
| ANON_LINK_LIMIT | number | 5 | Máximo de links anónimos por IP en 24h |
| ANON_LINK_TTL_HOURS | number | 24 | Horas antes de que un link anónimo expire |
...
| UPSTASH_REDIS_REST_URL | string | No* | URL de Upstash Redis (*rate limiting deshabilitado sin ella) |
| UPSTASH_REDIS_REST_TOKEN | string | No* | Token de Upstash Redis |
...
| claimLinkSchema | linkId | string | uuid() |
| claimLinkSchema | shortCode | string | min 5, max 12 |
